...
namespace Bau.Libraries.LibBlobStorage.Storage
{
	...
	public class AzureStorageBlobManager : ICloudStorageManager
	{
       ...
       /// <summary>
		...
		/// </summary>
        public async Task<System.IO.Stream> OpenReadAsync(string container, string blobFileName)
        {
            return await (await GetBlobClientAsync(container, blobFileName)).OpenReadAsync();
        }

		/// <summary>
		///		Abre un stream de escritura sobre un blob
		/// </summary>
        public async Task<System.IO.Stream> OpenWriteAsync(string container, string blobFileName, bool overwrite)
        {
			BlockBlobClient blockBlobClient = new BlockBlobClient(StorageConnectionString, container, blobFileName);
				...
				return await blockBlobClient.OpenWriteAsync(overwrite);
        }
